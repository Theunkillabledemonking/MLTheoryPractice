...
# 1. 데이터 로딩
# fetch_california_housing() 함수로 입렉 데이터(X)와 타겟값(y)을 불러옴
...
# SGD는 입력값 크기에 민감하므로 반드시 정규화 필요
scalar = StandardScaler()
X_train_scaled = scalar.fit_transform(X_train) # 훈련 데이터를 기준으로 스케일 조정
X_test_scaled = scalar.transform(X_test)       # 동일한 기준으로 테스트 데이터 변환

# 4. SGDRegressor 모델 정의 및 학습
model = SGDRegressor(
    max_iter=10000,              # 최대 반복 횟수 (epoch 수)
    tol=0.0001,                   # 손실이 tol보다 작아지면 학습 중단 (수렴 기준)
